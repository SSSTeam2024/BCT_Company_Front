...
import { useGetAllRequestedFeaturesByCompanyIDQuery } from "features/RequestFeature/requestFeatureSlice";

const RequestedFeatures = () => {
  document.title = "Requested Features | Company Administration";

  const user = useSelector((state: RootState) => selectCurrentUser(state));

  const { data: AllRequestedFeatures = [] } =
    useGetAllRequestedFeaturesByCompanyIDQuery(user?._id!);
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumb title="Requested Features" pageTitle="Help" />
          <Col lg={12}>
            <Card>
              <Card.Body>
